...
function remoteConnect() {
    if (process.env.REMOTE_URL) {
        ...
        try {
            remote_ws = new WebSocket(process.env.REMOTE_URL + "/socket/site/" + process.env.SITE_ID, ["soap", "wamp"]);

            remote_ws.on("close", function close() {
                startBroadcast = false
                remote_ws_open = false;
                sendGroup = true;
                LOGGER_MODEL.createProgramEvent("Connection", "Remote Connection disconnected", function () { });
                remote_ws.close();
                setTimeout(function () {
                    remoteConnect();
            ...
            remote_ws.on("open", function open() {
                LOGGER_MODEL.createProgramEvent("Connection", "Remote Connection connected", function () { });
                remote_ws_open = true;
                  ...
                  if (remote_ws.readyState === 1) {
                    ...
                    groups.getGroupConfigFiles(function (err, data) {
                      ...
                      data.forEach(function (val) {
                        ...
                        _v.version = process.env.APP_VERSION ? process.env.APP_VERSION : "";
                        _cont[_v.GroupID] = _v;
                      });
                      try {
                        var obj1 = {
                        ...
                        console.log("ERROR SENDING GROUPCONFIG ::: ", er);
                      }
                    });
                  }
                }, 3000);
            ...
            remote_ws.on("error", function error(err) {
                console.log("REMOTE ERROR not connected", err);
            });
            ...
            remote_ws.on("message", function (msg) {
                ...
                if (_msg["dynamic-security-update-rule"] && !_msg["api"]) {
                    ...
                    axios({
                        method: 'post',
                        url: 'http://' + pi_ip + '/api/dynamic-security/rule/'  + _msg.ruleID + '/delete',
                    }).then(function (response) {
                    }).catch(error => {
                        console.log("Rule not deleted")
                    });
                ...
                if (_msg["dynamic-security-deactivate-rule"] && !_msg["api"]) {
                    ...
                    axios({
                       ...
                       console.log('rule deactivated')
                        ...
                        console.log('rule not deactivated')
                    });
                }
                ...
                if (_msg["api"] && _msg["notControls"]) {
                    ...
                    if (DATA) {
                        try {
                            ...
                            console.log("REMOTE API ERROR :::", err.message);
                        }
                    } else {
...
var connect = function (pi_group, location) {
    ...
    var lastFaultsUpdated = TOOLS.getRedisKeyValue('lastFaultsUpdated') || [0,0,0,0,0,0,0,0]
    var lastAlarmsUpdated = TOOLS.getRedisKeyValue('lastAlarmsUpdated') || [0,0,0,0,0,0,0,0]
    var lastFaultId = TOOLS.getRedisKeyValue('lastFaultId') || [0,0,0,0,0,0,0,0]
    var lastAlarmId = TOOLS.getRedisKeyValue('lastAlarmId') || [0,0,0,0,0,0,0,0]

    var ows = new WebSocket(`ws://` + "0.0.0.0" + ":" + process.env.APP_PORT + "/socket/server");
    var ows_open = false;

    ows.on("close", function close() {
        ows_open = false;
        ows.close();
        //console.log('ows is closed')
    });
    ...
    ows.on("open", function open() {
        ...
        try {
            ...
            console.log("SETTINGS_MODEL_ERROR ::: ", err.message);
        }
        //console.log('ows is connected')
    });
    ...
    ws.on("open", function open() {
        LOGGER_MODEL.createProgramEvent("Connection", "RaspberryPi Connected. Location: " + location, function () { });
    ...
    ws.on("close", function close() {
        ...
        LOGGER_MODEL.createProgramEvent("Connection", "RaspberryPi Disconnected. Location: " + location, function () { });
        /*
    for (var x = 1; x <= 8; x++) {
      cServices[pi_group] = {};
    ...
    setTimeout(function () {
      ...
      connect(pi_group, location);
        ...
        setInterval(function () {
            ...
            // 	if(ws.readyState === 3){
            // 		console.log(wsLists);
            // 		// for(let x of wsLists){
            // 		// 	console.log(x);
            // 		// }
            // 		// wsCloseFunc(ws, pi_group, location, 1000);
            // 		}
            // 	}
            // catch (err) {
            // 	console.log('Sudden disconnection')
    ...
    var currentFloors = {}
    var currentModes = {"0":"offline","1":"offline","2":"offline","3":"offline","4":"offline","5":"offline","6":"offline","7":"offline"}
    ...
    ws.on("message", function incoming(data) {
        ...
        if (ows_open) {
            if (_d[pi_ip].MessageType === "Risers") {
                hallCalls = {};
                hallCalls = { ...hallCalls, pi_group: pi_group, MessageType: _d[pi_ip].MessageType, data: data };
            }
            if (Object.keys(hallCalls).length > 0) {
                try {
                    ...
                    if (remote_ws_open && remote_ws && remote_ws.readyState === 1) {     
                        hallCalls = { ...hallCalls, ...{ SiteID: process.env.SITE_ID, JobID: _d[pi_ip].JobID } };
                    ...
                    console.log("ERROR_SENDING_RISERS ::: ", err.message);
                }
            }
            try {
                ...
                ows.send(JSON.stringify({ pi_group: pi_group, MessageType: _d[pi_ip].MessageType, data: data }));
            } catch (err) {
                console.log("ERROR_SENDING_DATA  ::: ", err);
            }
            ...
            try {
                ...
                console.log("Websocket server has been disconnected");
            }
        }
        ...
        if (_d[pi_ip].MessageType == "Risers") {
            ...
            try {
                if (
                    ...
                    newHallCalls[pi_group] = getHallCalls(GROUP_FILES[pi_group]["Cars"][0]["FloorArray"], _d[pi_ip].Risers);
                }
            } catch (catch_error) {
                console.log("catch error ==> hallcalls");
            }


            var hcWaitUp = arrDiff(newHallCalls[pi_group]["up"], prevHallCalls[pi_group]["up"]);
            var hcWaitDown = arrDiff(newHallCalls[pi_group]["down"], prevHallCalls[pi_group]["down"]);
            ...
            var hallCallDiffUp = arrDiff(prevHallCalls[pi_group]["up"], newHallCalls[pi_group]["up"]);
            var hallCallDiffDown = arrDiff(prevHallCalls[pi_group]["down"], newHallCalls[pi_group]["down"]);
            ...
            //console.log("hallCallDiffUp",hallCallDiffUp);
            if (hallCallDiffUp.length > 0) {
                //console.log("up",hallCallDiffUp);
                LOGGER_MODEL.createHallCalls(pi_group, hallCallDiffUp, "up", function () { });
                startWait(pi_group, hallCallDiffUp, "up");
            }
            prevHallCalls[pi_group]["up"] = newHallCalls[pi_group]["up"];
            //console.log("hallCallDiffDown",hallCallDiffDown);
            if (hallCallDiffDown.length > 0) {
                //console.log("down",hallCallDiffDown);
                LOGGER_MODEL.createHallCalls(pi_group, hallCallDiffDown, "down", function () { });
        ...
        if (_d[pi_ip].MessageType == "Cars") {
            ...
            if(first_car && is_modified_dad_parameter) {
                TOOLS.setRedisKeyValue("requests:" + pi_group + ":last_updated_remote", parseInt(first_car.LastUpdateParameter))
            ...
            Object.keys(_cars).forEach(function (e, key) {
                ...
                currentFloors[_cars[e].CarID] = {
                    ...
                    Status: getDoorModeOperation(_cars[e].ClassOfOperation,_cars[e].ModeOfOperation)
                }

                var car_mode = getDoorModeOperation(_cars[e].ClassOfOperation, _cars[e].ModeOfOperation);
                currentModes[_cars[e].CarID] = car_mode



                if (_cars[e].Faults > 0) {
                    if (lastFaultId[e] != _cars[e].Faults || lastFaultId[e] === undefined) {
                        let params = {};
                        if (lastFaultsUpdated[e] != 0) {
                            params.date_time = parseInt(lastFaultsUpdated[e]);
                        }

                        axios.get(`http://` + pi_ip + `/api/faults/cars`, { params: params, timeout: 3000 })
                            .then((response) => {
                                ...
                                LOGGER_MODEL.updateRptFaultsTable(pi_group, response.data, function () { });
                            })
                            .catch((err) => {
                            });
                    }
                ...
                if (_cars[e].Alarms > 0) {
                    if (lastAlarmId[e] != _cars[e].Alarms || lastAlarmId[e] === undefined) {
                        let params = {};
                        if (lastAlarmsUpdated[e] != 0) {
                            params.date_time = parseInt(lastAlarmsUpdated[e]);
                        }
                        axios.get(`http://` + pi_ip + `/api/alarms/cars`, { params: params, timeout: 3000 })
                            .then((response) => {
                                ...
                                LOGGER_MODEL.updateRptAlarmsTable(pi_group, response.data, function () { });
                            })
                            .catch((err) => {
                            });
                    }
                ...
                try {
                    //console.log(typeof _cars[e].CarID,typeof GROUP_FILES[pi_group])
                    if (
                        typeof _cars[e].CarID != "undefined" &&
                        typeof GROUP_FILES[pi_group] != "undefined" &&
                        typeof GROUP_FILES[pi_group]["Cars"] != "undefined" &&
                        typeof GROUP_FILES[pi_group]["Cars"][_cars[e].CarID] != "undefined" &&
                        typeof GROUP_FILES[pi_group]["Cars"][_cars[e].CarID]["FloorArray"] != "undefined"
                    ) {
                        newCarCalls[pi_group][_cars[e].CarID] = getCarCalls(GROUP_FILES[pi_group]["Cars"][_cars[e].CarID]["FloorArray"], _cars[e]);
                    }
                } catch (catch_error) {
                    console.log("catch_error==>carcalls");
                }
                //console.log(getCarCalls(GROUP_FILES[pi_group]['Cars'][_cars[e].CarID]['FloorArray'],_cars[e]));
                var carCallDiff = arrDiff(prevCarCalls[pi_group][_cars[e].CarID], newCarCalls[pi_group][_cars[e].CarID]);
                //console.log("CARCALLDIFF",carCallDiff,prevCarCalls[pi_group][_cars[e].CarID],newCarCalls[pi_group][_cars[e].CarID])
                if (carCallDiff.length > 0) {
                    LOGGER_MODEL.createCarCalls(_cars[e].CarID + 1, pi_group, carCallDiff, function () { });
                }
                prevCarCalls[pi_group][_cars[e].CarID] = newCarCalls[pi_group][_cars[e].CarID];
                ...
                cServices[pi_group][_cars[e].CarID + 1]["ClassOfOperation"]["new"] = _cars[e].ClassOfOperation;
                cServices[pi_group][_cars[e].CarID + 1]["ModeOfOperation"]["new"] = _cars[e].ModeOfOperation;
                if (_cars[e].Faults === 0) {
                    if (
                        cServices[pi_group][_cars[e].CarID + 1]["ClassOfOperation"]["prev"] != cServices[pi_group][_cars[e].CarID + 1]["ClassOfOperation"]["new"] ||
                        cServices[pi_group][_cars[e].CarID + 1]["ModeOfOperation"]["prev"] != cServices[pi_group][_cars[e].CarID + 1]["ModeOfOperation"]["new"]
                    ) {
                        cServices[pi_group][_cars[e].CarID + 1]["ClassOfOperation"]["prev"] = cServices[pi_group][_cars[e].CarID + 1]["ClassOfOperation"]["new"];
                        cServices[pi_group][_cars[e].CarID + 1]["ModeOfOperation"]["prev"] = cServices[pi_group][_cars[e].CarID + 1]["ModeOfOperation"]["new"];
                    ...
                    cServices[pi_group][_cars[e].CarID + 1]["ModeOfOperation"]["prev"] = "faulted";
                }
    ...
    ws.on("error", function error(err) {
        console.log(location + "::error disconnected");
...
function wsCloseFunc(ws, pi_group, location, interval = 10000) {
    ...
    // console.log("IP LISTS", wsLists, pi_group);
    connectedRPI[pi_group] = false;
    _wsCloseInterval = setTimeout(function () {
        ...
        try {
            ...
            console.log("Socket disconnected/closed");
        }
        try {
...
function getCarCalls(carfloor, data) {
    ...
    for (var i = 0; i < buttonLength; i++) {
        ...
        // console.log(secureLatchedFront, i)

        //console.log('latched::',latchedFront,'floor::',i);
        var frontButton = i;
        var rearButton = i;
...
function carDoorsCapture(pi_group, _cars, e) {
    ...
    if (carDoors[pi_group][_cars[e].CarID + 1]["time_to_open"] == 0 && carDoors[pi_group][_cars[e].CarID + 1]["new"] == 2) {
        carDoors[pi_group][_cars[e].CarID + 1]["time_to_open"] = new Date();
    }

    if (carDoors[pi_group][_cars[e].CarID + 1]["time_open"] == 0 && carDoors[pi_group][_cars[e].CarID + 1]["new"] == 3) {
        carDoors[pi_group][_cars[e].CarID + 1]["time_open"] = new Date();
    }

    if (carDoors[pi_group][_cars[e].CarID + 1]["time_to_close"] == 0 && carDoors[pi_group][_cars[e].CarID + 1]["new"] == 5) {
        carDoors[pi_group][_cars[e].CarID + 1]["time_to_close"] = new Date();
    }

    if (carDoors[pi_group][_cars[e].CarID + 1]["new"] != carDoors[pi_group][_cars[e].CarID + 1]["prev"]) {
        var door_state = "";

        if (carDoors[pi_group][_cars[e].CarID + 1]["new"] == 3 && carDoors[pi_group][_cars[e].CarID + 1]["prev"] == 2) {
            var _tto = new Date().getTime() - carDoors[pi_group][_cars[e].CarID + 1]["time_to_open"].getTime();
            _tto = Math.abs(_tto / 1000);
            door_state = "time_to_open";
            LOGGER_MODEL.createDoorsReport(pi_group, _cars[e].CarID + 1, _cars[e].CurrentLanding, door_state, _tto, 'front');
            carDoors[pi_group][_cars[e].CarID + 1]["time_to_open"] = 0;
        }

        if (carDoors[pi_group][_cars[e].CarID + 1]["new"] == 1 && carDoors[pi_group][_cars[e].CarID + 1]["prev"] == 5) {
            var _ttc = new Date().getTime() - carDoors[pi_group][_cars[e].CarID + 1]["time_to_close"].getTime();
            _ttc = Math.abs(_ttc / 1000);
            door_state = "time_to_close";
            LOGGER_MODEL.createDoorsReport(pi_group, _cars[e].CarID + 1, _cars[e].CurrentLanding, door_state, _ttc, 'front');
            carDoors[pi_group][_cars[e].CarID + 1]["time_to_close"] = 0;
        }

        if (carDoors[pi_group][_cars[e].CarID + 1]["new"] == 5 && carDoors[pi_group][_cars[e].CarID + 1]["prev"] == 3) {
            var _to = new Date().getTime() - carDoors[pi_group][_cars[e].CarID + 1]["time_open"].getTime();
            _to = Math.abs(_to / 1000);
            door_state = "time_open";
            LOGGER_MODEL.createDoorsReport(pi_group, _cars[e].CarID + 1, _cars[e].CurrentLanding, door_state, _to, 'front');
            carDoors[pi_group][_cars[e].CarID + 1]["time_open"] = 0;
        }

        carDoors[pi_group][_cars[e].CarID + 1]["prev"] = carDoors[pi_group][_cars[e].CarID + 1]["new"];
    }
    ...
    if (rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_open"] == 0 && rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] == 2) {
        rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_open"] = new Date();
    }

    if (carDoors[pi_group][_cars[e].CarID + 1]["time_open"] == 0 && rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] == 3) {
        rearCarDoors[pi_group][_cars[e].CarID + 1]["time_open"] = new Date();
    }

    if (rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_close"] == 0 && rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] == 5) {
        rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_close"] = new Date();
    }

    if (rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] != rearCarDoors[pi_group][_cars[e].CarID + 1]["prev"]) {
        var door_state = "";

        if (rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] == 3 && rearCarDoors[pi_group][_cars[e].CarID + 1]["prev"] == 2) {
            var _tto = new Date().getTime() - rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_open"].getTime();
            _tto = Math.abs(_tto / 1000);
            door_state = "time_to_open";
            LOGGER_MODEL.createDoorsReport(pi_group, _cars[e].CarID + 1, _cars[e].CurrentLanding, door_state, _tto, 'rear');
            rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_open"] = 0;
        }

        if (rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] == 1 && rearCarDoors[pi_group][_cars[e].CarID + 1]["prev"] == 5) {
            var _ttc = new Date().getTime() - rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_close"].getTime();
            _ttc = Math.abs(_ttc / 1000);
            door_state = "time_to_close";
            LOGGER_MODEL.createDoorsReport(pi_group, _cars[e].CarID + 1, _cars[e].CurrentLanding, door_state, _ttc, 'rear');
            rearCarDoors[pi_group][_cars[e].CarID + 1]["time_to_close"] = 0;
        }

        if (rearCarDoors[pi_group][_cars[e].CarID + 1]["new"] == 5 && rearCarDoors[pi_group][_cars[e].CarID + 1]["prev"] == 3 && rearCarDoors[pi_group][_cars[e].CarID + 1]["time_open"] > 0) {
            var _to = new Date().getTime() - rearCarDoors[pi_group][_cars[e].CarID + 1]["time_open"].getTime();
            _to = Math.abs(_to / 1000);
            door_state = "time_open";
            LOGGER_MODEL.createDoorsReport(pi_group, _cars[e].CarID + 1, _cars[e].CurrentLanding, door_state, _to, 'rear');
            rearCarDoors[pi_group][_cars[e].CarID + 1]["time_open"] = 0;
        }

        rearCarDoors[pi_group][_cars[e].CarID + 1]["prev"] = rearCarDoors[pi_group][_cars[e].CarID + 1]["new"];
    }
...
function floorTfloor(pi_group, _cars, e) {
    ...
    if (_cars[e].CurrentLanding != _cars[e].DestinationLanding) {
        ...
        if (fTfData[pi_group][_cars[e].CarID + 1]["Timer"] == 0 && fTfData[pi_group][_cars[e].CarID + 1]["CurrentLanding"] == 0 && fTfData[pi_group][_cars[e].CarID + 1]["DestinationLanding"] == 0) {
            fTfData[pi_group][_cars[e].CarID + 1]["Timer"] = new Date();
            fTfData[pi_group][_cars[e].CarID + 1]["CurrentLanding"] = _cars[e].CurrentLanding;
            fTfData[pi_group][_cars[e].CarID + 1]["DestinationLanding"] = _cars[e].DestinationLanding;
        }
    } else {
        if (fTfData[pi_group][_cars[e].CarID + 1]["Timer"] != 0 && _cars[e].CarSpeed == 0) {
            var wt = new Date().getTime() - fTfData[pi_group][_cars[e].CarID + 1]["Timer"].getTime();
            wt = Math.abs(wt / 1000);
            var direction = fTfData[pi_group][_cars[e].CarID + 1]["CurrentLanding"] > fTfData[pi_group][_cars[e].CarID + 1]["DestinationLanding"] ? "down" : "up";
            LOGGER_MODEL.createFTF(pi_group, _cars[e].CarID + 1, fTfData[pi_group][_cars[e].CarID + 1]["CurrentLanding"], fTfData[pi_group][_cars[e].CarID + 1]["DestinationLanding"], direction, wt, function () { });
    ...
    floors.forEach(function (floor) {
        if (list_waited_floors[direction].includes(floor)) {
            ...
            if (index > -1) {
                ...
                if (floor >= 0 && pi_group && wTimes[pi_group][floor] && wTimes[pi_group][floor][direction] != "") {
                    var _time = new Date().getTime() - wTimes[pi_group][floor][direction].getTime();
                    _time = Math.abs(_time / 1000);
                    //wTimes[pi_group][floor][direction] = new Date();
                    LOGGER_MODEL.createWaitTime(pi_group, floor, direction, _time, max_floors);
    ...
    try {
        if (_inService[pi_group]) {
            ...
            Object.keys(_cars).forEach(function (e, key) {
                ...
                LOGGER_MODEL.createServices(_cars[e].CarID + 1, pi_group, _cars[e].CurrentLanding, _modeOfOperation, _classOfOperation, function () { });
            });
        }
    } catch (err) {
        console.log(err);
...
function updateFaultsAlarms(data) {
    ...
    if (hasFaultKey && data_cars.LastAlarmUpdateTime == "true" && preventUpdatingAlarms) {
        UPDATE_FAULTS_OR_ALARMS.update_file_from_pi('alarms');
        lastAlarmUpdateTime = now;
    }
    if (hasAlarmtKey && data_cars.LastFaultUpdateTime == "true" && preventUpdatingFaults) {
    ...
    if ((!hasFaultKey || !hasAlarmtKey) && !missingFaultAlarmKey) {
        console.log("Please check if you have the latest GUI or Middleware, the faults & alarms keys are missing !!!")
        setTimeout(() => {
