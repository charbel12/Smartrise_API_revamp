...
module.exports = {
  ...
  getCarLabelsFromConfigs: function (callback) {
    const fs = require("fs");
    const dirname = process.env.SETTINGS_GROUP_LOCATION + "/";
    let _labels = [];
          ...
          fs.readFile(dirname + filename, "utf-8", function (err, content) {
            ...
            try {
              ...
              if (json.Cars && Array.isArray(json.Cars)) {
                const labels = json.Cars.map((car) => {
                  let label = car.Label || "";
              ...
              // Call callback when all files processed
              if (filenames.length === _labels.length) {
                callback(null, _labels);
                return;
              }
            } catch (parseErr) {
  ...
  restore: function (callback) {
    ...
    var id = "1";
    var fileName =
      process.env.SETTINGS_GROUP_LOCATION + "/Group" + id + ".json";
      ...
      Cars: [
        {
          ...
          NumberOfFloors: 48,
          FloorArray: [
            { Pi: "G", DoorSide: 2, GroupFloorID: 0 },
            { Pi: "2", DoorSide: 2, GroupFloorID: 1 },
            { Pi: "3", DoorSide: 2, GroupFloorID: 2 },
            { Pi: "3A", DoorSide: 2, GroupFloorID: 3 },
            { Pi: "4", DoorSide: 2, GroupFloorID: 4 },
            { Pi: "6", DoorSide: 2, GroupFloorID: 5 },
            { Pi: "7", DoorSide: 2, GroupFloorID: 6 },
            { Pi: "8", DoorSide: 2, GroupFloorID: 7 },
            { Pi: "9", DoorSide: 2, GroupFloorID: 8 },
            { Pi: "10", DoorSide: 2, GroupFloorID: 9 },
            { Pi: "11", DoorSide: 2, GroupFloorID: 10 },
            { Pi: "12", DoorSide: 2, GroupFloorID: 11 },
            { Pi: "13", DoorSide: 2, GroupFloorID: 12 },
            { Pi: "14", DoorSide: 2, GroupFloorID: 13 },
            { Pi: "15", DoorSide: 2, GroupFloorID: 14 },
            { Pi: "16", DoorSide: 2, GroupFloorID: 15 },
            { Pi: "17", DoorSide: 2, GroupFloorID: 16 },
            { Pi: "18", DoorSide: 2, GroupFloorID: 17 },
            { Pi: "19", DoorSide: 2, GroupFloorID: 18 },
            { Pi: "20", DoorSide: 2, GroupFloorID: 19 },
            { Pi: "21", DoorSide: 2, GroupFloorID: 20 },
            { Pi: "22", DoorSide: 2, GroupFloorID: 21 },
            { Pi: "23", DoorSide: 2, GroupFloorID: 22 },
            { Pi: "24", DoorSide: 2, GroupFloorID: 23 },
            { Pi: "25", DoorSide: 2, GroupFloorID: 24 },
            { Pi: "26", DoorSide: 2, GroupFloorID: 25 },
            { Pi: "27", DoorSide: 2, GroupFloorID: 26 },
            { Pi: "28", DoorSide: 2, GroupFloorID: 27 },
            { Pi: "29", DoorSide: 2, GroupFloorID: 28 },
            { Pi: "30", DoorSide: 2, GroupFloorID: 29 },
            { Pi: "31", DoorSide: 2, GroupFloorID: 30 },
            { Pi: "32", DoorSide: 2, GroupFloorID: 31 },
            { Pi: "33", DoorSide: 2, GroupFloorID: 32 },
            { Pi: "34", DoorSide: 2, GroupFloorID: 33 },
            { Pi: "35", DoorSide: 2, GroupFloorID: 34 },
            { Pi: "36", DoorSide: 2, GroupFloorID: 35 },
            { Pi: "37", DoorSide: 2, GroupFloorID: 36 },
            { Pi: "38", DoorSide: 2, GroupFloorID: 37 },
            { Pi: "39", DoorSide: 2, GroupFloorID: 38 },
            { Pi: "40", DoorSide: 2, GroupFloorID: 39 },
            { Pi: "41", DoorSide: 2, GroupFloorID: 40 },
            { Pi: "42", DoorSide: 2, GroupFloorID: 41 },
            { Pi: "43", DoorSide: 2, GroupFloorID: 42 },
            { Pi: "44", DoorSide: 2, GroupFloorID: 43 },
            { Pi: "45", DoorSide: 2, GroupFloorID: 44 },
            { Pi: "46", DoorSide: 2, GroupFloorID: 45 },
            { Pi: "47", DoorSide: 2, GroupFloorID: 46 },
            { Pi: "48", DoorSide: 2, GroupFloorID: 47 },
          ],
          NumDoors: "1",
    ...
    fs.readdir(process.env.SETTINGS_GROUP_LOCATION, (err, files) => {
      ...
      for (const file of files) {
        fs.unlink(
          path.join(process.env.SETTINGS_GROUP_LOCATION, file),
          (err) => {
            if (err) throw err;
          }
        );
    ...
    exec(
      "rm -rf /files/Smartrise_Local_Monitor/SMARTRISE_API/configs/",
      (error, stdout, stderr) => {
        exec(
          "cp -r /data/default_configs/ /files/Smartrise_Local_Monitor/SMARTRISE_API/configs/",
          (error, stdout, stderr) => {
            exec(
              'redis-cli -h redis_lm --raw keys "*" | xargs redis-cli -h redis_lm del 2>&1 1>/dev/null',
              (error, stdout, stderr) => {
                callback("", "success");
              }
            );
  ...
  update: async function (all_config, callback) {
    ...
    Object.keys(all_config).forEach(async function (_id) {
      ...
      var fileName =
        process.env.SETTINGS_GROUP_LOCATION + "/Group" + _id + ".json";
      ...
      if (group_index < current_group_count) {
        ...
        if (body["FileName"] != "") {
          ...
          file = JSON.parse(
            await readFile("configs/groups/Group" + body["GroupID"] + ".json")
          );
      ...
      if (_id == Object.keys(all_config).length) {
        current_piFile["data"] = current_piFile["data"].splice(
          0,
          Object.keys(all_config).length
        );
        ...
        for (let index = _id + 1; index <= 8; index++) {
          if (
            fs.existsSync(
              process.env.SETTINGS_GROUP_LOCATION + "/Group" + index + ".json"
            )
          ) {
            fs.unlinkSync(
              process.env.SETTINGS_GROUP_LOCATION + "/Group" + index + ".json"
