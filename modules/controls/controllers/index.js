...
APP.get(`${VARS.base_route}/settings/:name/group/:group/car/:car`, async (req, res) => {
    try {
        ...
        if (!/^\d+$/.test(group)) throw new Error('Invalid group number!');
        if (!/^\d+$/.test(car)) throw new Error('Invalid car number!');
...
* @security JWT
*/
// APP.patch(`${VARS.base_route}/settings/:name/group/:group/car/:car`, async (req,res)=>{
//     try{
//         const {name, group, car} = req.params;
